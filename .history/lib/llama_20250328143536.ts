  ...
  try {
    ...
    try {
      ...
      const missingFields = requiredFields.filter(field => {
        const hasField = parsedResponse.hasOwnProperty(field);
        const fieldValue = parsedResponse[field as keyof OutfitReportResponse];
        const isValidValue = fieldValue !== null && fieldValue !== undefined;
        
        // Arrays should have at least one item
        if (isValidValue && Array.isArray(fieldValue)) {
          return fieldValue.length === 0;
        }
        
        // For the budget object, check if it exists but don't validate its contents
        if (field === "budget" && isValidValue) {
          return false; // budget exists, so it's not missing
        }
        
        return !hasField || !isValidValue;
      });
      ...
      if (missingFields.length > 0) {
        ...
        // Try to fix the response before falling back to sample
        const fixedResponse = { ...parsedResponse };
        
        // Add missing fields from sample data
        missingFields.forEach(field => {
          console.log(`Fixing missing field: ${field} with sample data`);
          ...
          switch (field) {
            ...
            case 'footwear':
            case 'headwear':
              (fixedResponse as any)[field] = sampleReport[field as keyof typeof sampleReport];
              break;
            case 'accessories':
            case 'colors':
            case 'stylingTips':
            case 'cultural':
              ...
              break;
            case 'budget':
              (fixedResponse as any)[field] = sampleReport.budget;
              break;
          }
        });
        
        return fixedResponse as OutfitReportResponse;
      }
